...
class OsrController extends Controller
{
    ...
    public function actionAddSales()
    {
        ...
        if ($modelCustomer->load(Yii::$app->request->post()) && $model->load(Yii::$app->request->post())) {
            ...
            $modelCustomer->gender = strtolower($_POST['Customer']['gender']);
            ...
            $model->product_id = json_encode($_POST['product_id']);
            $model->quantity = json_encode($_POST['quantity']);
            $model->collectible_amount = json_encode($_POST['collectible_amount']);
