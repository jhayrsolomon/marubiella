        ...
        if(isset($_POST['date_sales'])){
            if($_POST['date_sales'] == 'sales_all'){
                ...
                if($_POST['date_sales'] == 'sales_today'){
                    $today = date('Y-m-d');
                    $sales = Yii::$app->marubiella->createCommand('SELECT from_unixtime( UNIX_TIMESTAMP(sales_product.date_created), "%Y-%m-%d") as pdate, sales_product.product_id, SUM(sales_product.quantity) as sum_qty FROM sales_product INNER JOIN sales_online ON sales_product.sales_online_id = sales_online.id WHERE sales_online.sales_status_id = 2 and sales_online.date_created like "'.$today.'" AND sales_online.employee_id = '.$employee->id.' GROUP BY sales_product.product_id, from_unixtime( UNIX_TIMESTAMP(sales_product.date_created), "%Y-%m-%d") ORDER BY from_unixtime( UNIX_TIMESTAMP(sales_product.date_created), "%Y-%m-%d"), sales_product.product_id ASC')->queryAll();
    ...
    public function actionViewSales()
    {
        ...
        if(isset($_POST['view_date_sales'])){
            if($_POST['view_date_sales'] == 'view_sales_all'){
                ...
                $teamSalesValidated = SalesOnline::find()->where(['sales_status_id'=>2])->all();
                $teamsales = SalesOnline::find()->where(['<>', 'sales_status_id', 2])->orderBy('sales_status_id ASC')->all();
            } else {
                if($_POST['view_date_sales'] == 'view_sales_today'){
                    ...
                    $teamSalesValidated = SalesOnline::find()->where(['sales_status_id'=>2])->andWhere(['like', 'date_created', $today])->all();
                    $teamsales = SalesOnline::find()->where(['like', 'date_created', $today])->andWhere(['<>', 'sales_status_id', 2])->orderBy('sales_status_id ASC')->all();
                } else {
                    ...
                    $teamSalesValidated = SalesOnline::find()->where(['sales_status_id'=>2])->andWhere(['between', 'date_created', $start_Date, $end_Date])->all();
                    $teamsales = SalesOnline::find()->where(['between', 'date_created', $start_Date, $end_Date])->andWhere(['<>', 'sales_status_id', 2])->orderBy('sales_status_id ASC')->all();
                }
            }
            ...
            $today = date('Y-m-d');
            $salesValidated = SalesOnline::find()->where(['employee_id' => $employee->id, 'sales_status_id'=>2])->andWhere(['like', 'date_created', $today])->all();
            $sales = SalesOnline::find()->where(['employee_id' => $employee->id])->andWhere(['<>', 'sales_status_id', 2])->andWhere(['like', 'date_created', $today])->orderBy('sales_status_id ASC')->all();
            
            $teamSalesValidated = SalesOnline::find()->where(['sales_status_id'=>2])->andWhere(['like', 'date_created', $today])->all();
            $teamsales = SalesOnline::find()->where(['<>', 'sales_status_id', 2])->andWhere(['like', 'date_created', $today])->orderBy('sales_status_id ASC')->all();
        }
        
        return $this->render('crud/view-sales', [
            'salesValidated' => $salesValidated,
            'sales' => $sales,
            'teamSalesValidated' => $teamSalesValidated,
            'teamsales' => $teamsales,
            'employee' => $employee,
        ]);
    }
