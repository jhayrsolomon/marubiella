...
/*use frontend\modules\models\Region;
use frontend\modules\models\Province;
use frontend\modules\models\Municipality;
use frontend\modules\models\Barangay;*/
...
/**
 * MasterListController implements the CRUD actions for Employee model.
 */
class MasterListController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $address->load(Yii::$app->request->post()) && $affiliation->load(Yii::$app->request->post())) {
            //$model->user_code = 2;
            //$model->id = 2;
            //$model->date_of_birth = date('Y-m-d',$_POST['date_of_birth']);
            $model->save();
        ...
        /*$province = ArrayHelper::map(Province::find()->all(), 'province_id', 'description');
        $municipality = ArrayHelper::map(Municipality::find()->all(), 'municipality_id', 'description');
        $barangay = ArrayHelper::map(Barangay::find()->all(), 'barangay_id', 'description');*/
